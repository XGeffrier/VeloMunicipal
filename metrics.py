...
def bike_ratio_2026_by_party(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    df["ratio"] = df["longueur_piste_2026"] / df["longueur_route"] * 100
...
towns_10k = complete_df[(complete_df["population"] >= 10_000) & (complete_df["population"] < 100_000)]
small_towns = complete_df[complete_df["population"] < 10_000]
...
avg_small_towns_ratio_2026 = small_towns["longueur_piste_2026"].sum() / small_towns["longueur_route"].sum()
